...
export default class SharepointAPIConnector  {
  ...
  public static getListItemsDetails(_context: WebPartContext, listSite: string, listName: string, listFilter: string): Promise<any> {
    return _context.spHttpClient.get(listSite + `/_api/web/lists/getbytitle('${listName}')/items?$Filter=${listFilter}&$top=4000`, SPHttpClient.configurations.v1)
